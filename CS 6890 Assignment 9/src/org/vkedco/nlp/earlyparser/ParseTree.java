...
public class ParseTree {
    ...
    public void display(StringBuffer buffer) {
        displayWithIndentation(this, 0, buffer);
    }

    private void displayWithIndentation(ParseTree pt, int numSpaces, StringBuffer buffer) {
        String indentStr = "";
        //for(int i = 0; i < numSpaces; i++)
            //indentStr += " ";
        System.out.print(indentStr);
        System.out.println(pt.mCFProduction.mLHS.toString() + " ");
        buffer.append(" " + pt.mCFProduction.mLHS.toString() + " ");
        if ( pt.children.size() == 0 ) {
            System.out.println(indentStr + " " + pt.mCFProduction.mRHS.get(0).toString());
            buffer.append(indentStr + " " + pt.mCFProduction.mRHS.get(0).toString());
            return;
        }
        Iterator<ParseTree> iter = pt.children.iterator();
        while ( iter.hasNext() ) {
            displayWithIndentation(iter.next(), numSpaces+1, buffer);
        }
    }
}